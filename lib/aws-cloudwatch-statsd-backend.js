...
function CloudwatchBackend(startupTime, config, emitter) {
  ...
  if (this.config.iamRole) {
    ...
    } else if (this.config.iamRole == 'ecs') {
      // If the iamRole is set to ecs, then attempt to fetch task role credentials
      ms = new AWS.ECSCredentials();
      ms.refresh(function(err) {
        if (err) { console.log('Failed to fetch IAM task role credentials: ' + err); }
        self.config.credentials = ms;
        setEmitter();
      });
    } else {
...
CloudwatchBackend.prototype.isBlacklisted = function(key) {
  ...
  if (this.config.whitelist && this.config.whitelist.length > 0) {
      if (this.config.whitelist.indexOf(key) >= 0) {
          return false;
      }
      return true;
  }
